...
| total_harga | Decimal(10,2) | Total harga yang dibayar |
...
| ```users.profile_id``` | ```user_profiles.id``` | one to one| Satu user hanya punya satu profil (biodata tambahan) dan sebaliknya |
...
| ```users.id``` | ```buku.id via buku_user_likes.user_id, buku_user_likes.buku_id``` | many to many| Banyak user bisa menyukai banyak buku, dan satu buku bisa disukai banyak user
